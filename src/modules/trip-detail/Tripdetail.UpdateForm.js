...
function TripDetailUpdateForm({ headerTitle, destinationId, onCloseSubmit, onClose, endTime, startTime }) {
    ...
    const loadData = async () => {
        axios.get(AppUntil.RequestApi("/destination/show/" + destinationId), await AppUntil.RequsetHeader()).then(
            (res) => {
                if (res?.data?.isSuccess) {
                    const results = res?.data?.results
                    setInitialvalues({
                        ...results,
                        price: _.toString(results.price),
                        time: moment(results.time).format("DD-MM-YYYY")
                    })
                }
            }
        )
    ...
    const onUpdateDes = async () => {
        if (checkValid(initialvalues)) {
            const values = {
                ...initialvalues,
                time: moment(initialvalues.time, "DD-MM-YYYY")
            }
            axios.post(AppUntil.RequestApi("/destination/update/" + destinationId), values, await AppUntil.RequsetHeader()).then(
                (res) => {
                    if (res.data?.isSuccess) {
    ...
    return (
        <View>
            ...
            <View style={{ padding: 0, marginTop: 8 }}>
                <View style={{ marginBottom: 10 }}>
                    <SafeAreaView >
                        <GooglePlacesInput opPressPlace={onPressPlace} placeName={initialvalues.place} />
                    </SafeAreaView>
...
export default TripDetailUpdateForm;